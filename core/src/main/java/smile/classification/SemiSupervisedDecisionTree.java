...
import io.github.sudarsun.ml.cre.ClassRatioEstimator;
 ...
 * @author Haifeng Li
 */
public class SemiSupervisedDecisionTree implements SoftClassifier<double[]>, Serializable {
    ...
    /// Class Ratio estimator for predicting the class proportions of an unlabeled dataset.
    private ClassRatioEstimator cre = new ClassRatioEstimator();
    ...
    public static class Trainer extends ClassifierTrainer<double[]> {
        ...
        @Override
        public SemiSupervisedDecisionTree train(double[][] x, int[] y) {
            return new SemiSupervisedDecisionTree(attributes, x, y, maxNodes, nodeSize, rule);
        }
    }
    ...
    class TrainNode implements Comparable<TrainNode> {
         ...
         * Constructor.
         */
        public TrainNode(Node node, double[][] x, int[] y, int[] samples, int nodeSize) {
            this.node = node;
            this.x = x;
            this.y = y;
        ...
        public boolean findBestSplit() {
            ...
            ArrayList<Integer> unlabeledItems = new ArrayList<Integer>();
            ...
            for (int i = 0; i < x.length; i++) {
                if (samples[i] > 0) {
                    if (y[i] != 0)
                        ...
                        unlabeledItems.add(i);
                }
            }

            if (!unlabeledItems.isEmpty()) {
                int ids[] = new int[unlabeledItems.size()];
                int i = 0;
                for (int item : unlabeledItems) {
                    ids[i++] = item;
                }

                double props[] = cre.estimateClassRatio(ids);
                for (i = 1; i <= props.length; i++) {
                    count[i] += (int) (props[i - 1] * (double) unlabeledItems.size());

                    // check for pure nodes after CRE.
            ...
            if (attributes[j].getType() == Attribute.Type.NUMERIC) {
                ...
                for (int i : order[j]) {
                    if (samples[i] > 0) {
                        ...
                        // if the data point is from the unlabeled data,
                        // handle it appropriately.
                        if (y[i] == 0) {
                            unlabeledIds.add(i);
                        }

                        if (Double.isNaN(prevx) || x[i][j] == prevx /*|| (y[i] != 0 && y[i] == prevy)*/) {
                            prevx = x[i][j];
                            //prevy = y[i];
                        ...
                        if (!unlabeledIds.isEmpty()) {
                            if (unlabeledIds.size() > this.nodeSize) {
                                int ids[] = new int[unlabeledIds.size()];
                                for (int k = 0; k < ids.length; ++k)
                                    ids[k] = unlabeledIds.get(k);

                                //System.err.println("variable=" + j + " value=" + x[i][j] + " n=" + n + " unlabeled=" + unlabeledIds.size());

                                double prop[] = cre.estimateClassRatio(ids);
                                for (int k = 1; k <= prop.length; ++k)
                                    trueCount[k] += (int) (prop[k - 1] * ids.length);
                            }
                            else n -= trueCount[0];
                ...
                // we don't care about this part, for now!
                if (!unlabeledIds.isEmpty()) {
        ...
        public boolean split(PriorityQueue<TrainNode> nextSplits) {
            ...
            TrainNode trueChild = new TrainNode(node.trueChild, x, y, trueSamples, this.nodeSize);
            ...
            TrainNode falseChild = new TrainNode(node.falseChild, x, y, samples, this.nodeSize);
     ...
     * @param maxNodes the maximum number of leaf nodes in the tree.
     */
    public SemiSupervisedDecisionTree(double[][] x, int[] y, int maxNodes) {
     ...
     * @param rule the splitting rule.
     */
    public SemiSupervisedDecisionTree(double[][] x, int[] y, int maxNodes, SplitRule rule) {
     ...
     * @param rule the splitting rule.
     */
    public SemiSupervisedDecisionTree(double[][] x, int[] y, int maxNodes, int nodeSize, SplitRule rule) {
     ...
     * @param maxNodes the maximum number of leaf nodes in the tree.
     */
    public SemiSupervisedDecisionTree(Attribute[] attributes, double[][] x, int[] y, int maxNodes) {
     ...
     * @param rule the splitting rule.
     */
    public SemiSupervisedDecisionTree(Attribute[] attributes, double[][] x, int[] y, int maxNodes, SplitRule rule) {
     ...
     * @param rule the splitting rule.
     */
    public SemiSupervisedDecisionTree(Attribute[] attributes, double[][] x, int[] y, int maxNodes, int nodeSize, SplitRule rule) {
     ...
     */
    public SemiSupervisedDecisionTree(Attribute[] attributes, double[][] x, int[] y, int maxNodes, int nodeSize, int mtry, SplitRule rule, int[] samples, int[][] order) {
        ...
        // build the cre model.
        this.cre.buildModel(x, y);
        if (!this.cre.isOK())
            throw new IllegalArgumentException("semi supervised decision tree cannot be built when unlabeled data is not available!");

        // class label set.
        int[] labels = Math.unique(y);
        Arrays.sort(labels);
        
        // this would never happen, but nonetheless...
        if (labels[0] != 0)
            throw new IllegalArgumentException("unlabeled data points should have the label set as '0'");
        ...
        k = labels.length;
        if (k < (2+1)) { // excluding the 0-label
        ...
        int labeledCount = n - unlabeledCount;

        // collect the unlabeled items.
        int [] unlabeledIds = new int[unlabeledCount];
        int j = 0;
        for (int i = 0; i < n && j < unlabeledCount; i++) {
            if (y[i] == 0)
                unlabeledIds[j++] = i;
        }

        double [] props = cre.estimateClassRatio(unlabeledIds);

        for (int i = 1; i < k; ++i)
            count[i] += (int)((double)unlabeledCount * props[i-1]);
        ...
        TrainNode trainRoot = new TrainNode(root, x, y, samples, this.nodeSize);
    ...
    public String dot() {
        ...
        builder.append("digraph SemiSupervisedDecisionTree {\n node [shape=box, style=\"filled, rounded\", color=\"black\", fontname=helvetica];\n edge [fontname=helvetica];\n");
